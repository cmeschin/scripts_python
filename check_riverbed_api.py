...
def get_existing_errors(dict, ipaddr):
    """
    Function to get nbErrors saved on last check
    :return:
    """
    try:
        nbErrors = dict[ipaddr][2]
        # print("dict: {} {}".format(nbErrors, dict[ipaddr]))
    except KeyError:
        nbErrors = 0
...
def get_is_exclude(exclude, ipAddr):
    """
...
def get_ip_addr(value):
    """
    Function to extract ipAddr in line
    :return:
        ipAddr: ip address
    """
...
def get_values(line):
    ...
    example:
        ...
        ['False',       'none',         '0',        'flow_type="SFlow" flows_received_last_min="15" ipaddr="10.62.101.10" last_min="1582726323" name="N/A" status="N/A" timeslice_violation="false" versions="5"'   ]
    :return:
        isExclude
        nomCluster
        nbErrors
...
def stat_flow(exclude):
    ...
    """
     ...
     Création du dictionnaire de traitement
        source: 'flow_type="SFlow" flows_received_last_min="15" ipaddr="10.62.101.10" last_min="1582726323" name="N/A" status="N/A" timeslice_violation="false" versions="5"'
        ...
        exemple non cluster: {"10.62.101.10": ['False', 'none', '0', 'flow_type="SFlow" flows_received_last_min="15" ipaddr="10.62.101.10" last_min="1582726323" name="N/A" status="N/A" timeslice_violation="false" versions="5"']}
        exemple est cluster: {"10.62.101.10": ['True', 'CLUSTER1', '0', 'flow_type="SFlow" flows_received_last_min="15" ipaddr="10.62.101.10" last_min="1582726323" name="N/A" status="N/A" timeslice_violation="false" versions="5"']}
    """
    ipUpdate = []
    for line in statsFlow:
        ...
        for value in values:
            # updated = False
            if "ipaddr=" in value:
                ipAddr = get_ip_addr(value)
        # Liste toutes les IP actives
        ipUpdate.append(ipAddr)

        # Vérifie si l'ip fait partie d'un cluster
        isCluster, nomCluster = get_is_cluster(ipAddr)
        # print("isCluster: {} {} {}".format(ipAddr, isCluster, nomCluster))

        # Vérifie si l'ip fait partie des exclusions
        isExclude = get_is_exclude(exclude, ipAddr)
        # print("isExclude: {} {}".format(ipAddr, isExclude))

        # récupère le nombre d'erreurs du check précédent s'il existe
        nbErrors = get_existing_errors(dictListIP, ipAddr)

        # Ajoute ou mets à jour la liste
        dictListIP[ipAddr] = [isExclude, nomCluster, nbErrors, line]
        # print("ligne traitée {}: {}".format(ipAddr, dictListIP[ipAddr]))
    ...
    for ipAddr, value in dictListIP.items():
    ...
    for ip, values in dictListIP.items():  # pour chaque entrée
        ...
        if isCluster:  # si c'est un cluster
            ...
            if int(dictListIP[ip1][2]) >= 3 and int(dictListIP[ip2][2]) >= 3:  # si les deux ip du cluster sont à plus de 3 erreurs on remonte l'alerte
...
def stat_licencing(warning, critical):
    ...
    if prct_flow >= critical:
        status = "CRITICAL"
        message = "Le flux reçu sur la dernière minute ({} soit {}%) est supérieur à {}% de la capacité totale de la licence ({}).".format(flow, prct_flow, critical, capacity)
        detail = ""
        perfdata = "flow={};{};{};0;{}".format(flow, warn_flow, crit_flow, capacity)
    elif prct_flow >= warning:
        status = "WARNING"
        message = "Le flux reçu sur la dernière minute ({} soit {}%) est supérieur à {}% de la capacité totale de la licence ({}).".format(flow, prct_flow, warning, capacity)
        detail = ""
        perfdata = "flow={};{};{};0;{}".format(flow, warn_flow, crit_flow, capacity)
    else:
        status = "OK"
        message = "Le flux reçu sur la dernière minute ({} soit {}%) est inférieur à {}% de la capacité totale de la licence ({}).".format(flow, prct_flow, warning, capacity)
        detail = ""
        perfdata = "flow={};{};{};0;{}".format(flow, warn_flow, crit_flow, capacity)
    centreon_status.exit(status, message, detail, perfdata)
